    ...
    <Link  href={`/inventory/${link}/${params.id}`} className={`dark:bg-primary ${params.featured && "border-2 border-secondary"} bg-primary h-32 md:h-40 md:px-2  flex rounded-xl dark:text-white text-nowrap hover:text-wrap`}>
      ...
      <div className='w-1/4 flex-shrink-0 flex-grow-0 flex flex-col justify-between p-2'>
          <div>
            ...
            <div className=""></div>
             <Link href={`/inventory/edit-car/${params.id}`}>

              <button className="btn block  w-full my-1 bg-primary-light dark:bg-primary text-xs md:text-base p-0">Edit Car</button>
            </Link>
