...
const CarCard = (params) => {
  ...
  return (
    <>
    <Link key={params.key2}  href={`/inventory/${link}/${params.id}`} className={`dark:bg-primary ${params.featured && "border-2 border-secondary"} bg-primary h-28 mx-0 md:h-40 md:px-2 w-full flex rounded-xl dark:text-white text-nowrap hover:text-wrap`}>
      <div className='sm:w-1/3 lg:w-1/4 md:px-2 md:py-3 h-full flex-shrink-0 flex-grow-0'>
